...
void drawButton1(); // Кнопка, предназначенная для выполнения задачи Коммивояжера
...
public:
	...
	int task_Komovoizhora(); // Алгоритм задачи Коммивояжера
...
void Graph::InsertEdge(const int& vertex1, const int& vertex2, int weight)
{
	if (GetVertPos(vertex1) != (-1) && GetVertPos(vertex2) != (-1))
	{
		...
		if (adj_matrix[vertPos1][vertPos2] != 0 && adj_matrix[vertPos2][vertPos1] != 0)
		{
...
int Graph::task_Komovoizhora() // алгоритм выполнения задания Коммивояжера
{
	...
	//1.нахождение минимумов по строкам
	...
	cout << "С учётом минимумов по строкам: \n";
	for (int i = 0; i < amount_vertex; i++)
	{
		cout << i + 1 << "   ";
		for (int j = 0; j < amount_vertex; j++)
	...
	for (int i = 0; i < amount_vertex; i++)
	{
		...
		for (int j = 0; j < amount_vertex; j++)
		{
			if (adjMatrix_Komivoiazhor[j][i] < Min)
			{
				Min = adjMatrix_Komivoiazhor[j][i];
			}
		}
		dj[i] = Min;
	}
	...
	cout << "С учётом минимумов по столбцам: \n";
	for (int i = 0; i < amount_vertex; i++)
	{
		cout << i + 1 << "   ";
		for (int j = 0; j < amount_vertex; j++)
	...
	for (int i = 0; i < amount_vertex; i++)
	{
		//Вычисляем сколько нулей в строке
		int counter = 0;
	...
	for (int i = 0; i < amount_vertex; i++) {
		//Вычисляем сколько нулей в столбце
		int counter = 0;
	...
	//Вместе 0 подставляем сумму минимумов по строке и столбцу
	for (int i = 0; i < amount_vertex; i++)
	{
		for (int j = 0; j < amount_vertex; j++)
		{
	...
	cout << "С учётом оценок нулевых значений:\n";
	for (int i = 0; i < amount_vertex; i++)
	{
		cout << i + 1 << "   ";
		for (int j = 0; j < amount_vertex; j++)
	...
	//4. Ищем максимальную из нулевых строк
	...
	// Сравниваем найденное максимальное нулевое значение с tmp_vector чтобы указать, каким путям принадлежит данное значение
	int global_exit = 0;
	int global_i = -1;
	int global_j = -1;
...
void drawButton1() { // Поле для кнопки, предназначенной для выполнения алгоритма 
	glColor3f(1.9f, 1.9f, 0.7f);

	glBegin(GL_QUADS);
	glVertex2i(0, WinH);
	glVertex2i(0, WinH - WinH / 15);
	glVertex2i(WinW / 10, WinH - WinH / 15);
	glVertex2i(WinW / 10, WinH);
	glEnd();
	...
	string but1 = "Zadacha";
	string but2 = "Komivoyazhera";
	glRasterPos2i(4, WinH - 12);
	for (int i = 0; i < but1.length(); i++)
		glutBitmapCharacter(GLUT_BITMAP_9_BY_15, but1[i]);
	glRasterPos2i(4, WinH - 32);
	for (int i = 0; i < but2.length(); i++)
		glutBitmapCharacter(GLUT_BITMAP_9_BY_15, but2[i]);
}
...
void mouseClick(int button, int state, int x, int y) // Действие кнопки для выполнения задачи Коммивояжера
{
	if (x <= (WinW / 6) and y <= (WinH / 6)) // 1 кнопка
		if (button == GLUT_LEFT_BUTTON && state == GLUT_UP)
		{
			int k = gr.task_Komovoizhora();
			if (k == -1)
				cout << "Error";
		}
}
